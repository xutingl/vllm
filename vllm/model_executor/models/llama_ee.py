...
class EELlamaModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        ee_config = vllm_config.ee_config
        ...
        ramp_config = self.ee_config.ramp_config
        if ramp_config:
            for layer_idx, exit_threshold in ramp_config.items():
                print(f"[EELlamaModel] ramp_config: layer_idx: {layer_idx}. exit_threshold: {exit_threshold}")
...
class EELlamaForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def _init_model(
        ...
        return EELlamaModel(vllm_config=vllm_config, prefix=prefix, layer_type=layer_type)
